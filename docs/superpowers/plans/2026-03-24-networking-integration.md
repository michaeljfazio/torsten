...
**Goal:** Integrate the new `torsten-network` crate (feature/networking-rewrite branch) with `torsten-node` so the full workspace builds, tests pass, and the node runs against preview testnet.
...
**Tech Stack:** Rust, tokio, torsten-network (new API), torsten-node, torsten-cli
...
The integration touches 4 files in torsten-node plus the workspace Cargo.toml. Changes must be done in order because each phase depends on the previous:

```
Phase 1: serve.rs (trait adapters — lowest risk, traits preserved)
Phase 2: query.rs (query types — move types into torsten-network or torsten-node)
...
**Files modified in torsten-node:**
- `crates/torsten-node/src/node/serve.rs` — Update import paths (traits preserved)
- `crates/torsten-node/src/node/query.rs` — Move query snapshot types to torsten-network or inline
- `crates/torsten-node/src/node/mod.rs` — Replace server construction, peer management, connection handling
- `crates/torsten-node/src/node/sync.rs` — Replace PipelinedPeerClient with new ChainSync client

**Files modified in torsten-network:**
- `crates/torsten-network/src/lib.rs` — Add re-exports for backward compatibility
- `crates/torsten-network/src/protocol/chainsync/server.rs` — Re-export BlockAnnouncement
...
- Modify: `crates/torsten-network/src/lib.rs` (add UtxoQueryProvider re-export)
...
The new crate already has `UtxoQueryProvider` and `UtxoSnapshot` at the top level. But serve.rs imports them from `torsten_network::query_handler`. Add a compatibility re-export module:

```rust
// In crates/torsten-network/src/lib.rs, add:
/// Backward-compatible re-exports for query handler types.
/// These were previously in torsten_network::query_handler.
...
The old code references `torsten_network::query_handler::MultiAssetSnapshot` at line 431. This type alias is now `Vec<PolicyAssets>` which is `Vec<(Vec<u8>, Vec<(Vec<u8>, u64)>)>` — same underlying type.

- [ ] **Step 4: Verify serve.rs compiles**

Run: `cargo check -p torsten-node 2>&1 | grep -c "serve.rs"` — should be 0 (no errors from serve.rs)

- [ ] **Step 5: Commit**

```bash
git add crates/torsten-network/src/lib.rs crates/torsten-node/src/node/serve.rs
git commit -m "fix(node): update serve.rs to use new networking API import paths"
```

---

## Task 2: Move Query Snapshot Types to torsten-node

**Files:**
- Modify: `crates/torsten-node/src/node/query.rs`
- Create: `crates/torsten-node/src/node/query_types.rs`

The old `torsten_network::query_handler` module contained ~20 snapshot types used exclusively by query.rs to build N2C responses. These types don't belong in the network crate — they're ledger-specific data shapes. Move them into the node crate.

- [ ] **Step 1: Create query_types.rs with all snapshot types**

Extract from the old `query_handler/types.rs` (available in git history at `HEAD~18:crates/torsten-network/src/query_handler/types.rs`):
- `SnapshotStakeData`
- `PoolParamsSnapshot`
- `RelaySnapshot`
...
```bash
git show HEAD~18:crates/torsten-network/src/query_handler/types.rs > /tmp/old_types.rs
...
In `crates/torsten-node/src/node/mod.rs`, add:
```rust
...
Run: `cargo check -p torsten-node 2>&1 | grep "query.rs" | head -5`

- [ ] **Step 5: Commit**

```bash
git add crates/torsten-node/src/node/query_types.rs crates/torsten-node/src/node/query.rs crates/torsten-node/src/node/mod.rs
git commit -m "refactor(node): move query snapshot types from torsten-network to torsten-node"
```

---

## Task 3: Move Query Encoding Logic to torsten-node

**Files:**
- Modify: `crates/torsten-node/src/node/query.rs`
- Create: `crates/torsten-node/src/node/query_encoding.rs`
...
Run: `cargo check -p torsten-node 2>&1 | grep "query" | head -10`

- [ ] **Step 5: Commit**

```bash
git add crates/torsten-node/src/node/query_encoding.rs crates/torsten-node/src/node/query.rs crates/torsten-node/src/node/mod.rs
...
**Files:**
- Modify: `crates/torsten-node/src/node/mod.rs` (~3211 lines, heavy changes)
...
Old: `torsten_network::BlockAnnouncement` / `RollbackAnnouncement`
New: `torsten_network::protocol::chainsync::server::BlockAnnouncement`

Add re-exports in lib.rs for backward compatibility.

- [ ] **Step 8: Verify mod.rs compiles (expect sync.rs errors)**

Run: `cargo check -p torsten-node 2>&1 | grep -v "sync.rs" | tail -20`

- [ ] **Step 9: Commit**

```bash
git add crates/torsten-node/src/node/mod.rs crates/torsten-network/src/lib.rs
...
**Files:**
- Modify: `crates/torsten-node/src/node/sync.rs` (~3243 lines)

Replace the old `PipelinedPeerClient` and `NodeToNodeClient` with the new `PipelinedChainSyncClient` from `torsten_network::protocol::chainsync::client`.

- [ ] **Step 1: Replace sync client types**

Old imports:
```rust
use torsten_network::{BlockFetchPool, ChainSyncEvent, EbbInfo, HeaderBatchResult, NodeToNodeClient, PipelinedPeerClient};
```

New imports:
```rust
use torsten_network::protocol::chainsync::client::{ChainSyncEvent, PipelinedChainSyncClient};
...
Run: `cargo check -p torsten-node 2>&1 | tail -20`
...
- Modify: `crates/torsten-cli/Cargo.toml` (if it has torsten-network dep)
...
Confirm no other crate uses it (torsten-cli doesn't import from torsten-network).
...
```bash
./target/release/torsten-node mithril-import --network-magic 2 --database-path ./db-preview
```
...
- N2C queries work (via torsten-cli)
