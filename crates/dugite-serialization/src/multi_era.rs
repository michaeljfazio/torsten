...
fn decode_block_header(
    ...
    let header_hash = pallas_hash_to_dugite32(&block.hash());
    let pallas_header = block.header();

    let prev_hash = pallas_header
        .previous_hash()
        .map(|h| pallas_hash_to_dugite32(&h))
    ...
    let (
        ...
        (
            ...
            nonce_eta,
            nonce_vrf_proof,
            pallas_hash_to_dugite32(&hb.block_body_hash),
            OperationalCert {
...
fn decode_transaction_from_pallas_with_mode(
    ...
    let tx_hash = pallas_hash_to_dugite32(&tx.hash());
    ...
    let (era, raw_body_cbor, raw_witness_cbor) = match tx {
        PallasTx::Byron(_) => (dugite_primitives::era::Era::Byron, None, None),
        PallasTx::AlonzoCompatible(cow_tx, pallas_era) => {
            let era = match pallas_era {
                pallas_traverse::Era::Shelley => dugite_primitives::era::Era::Shelley,
                pallas_traverse::Era::Allegra => dugite_primitives::era::Era::Allegra,
                pallas_traverse::Era::Mary => dugite_primitives::era::Era::Mary,
                pallas_traverse::Era::Alonzo => dugite_primitives::era::Era::Alonzo,
                _ => dugite_primitives::era::Era::Conway,
            };
        ...
        PallasTx::Conway(cow_tx) => {
            let body_cbor = Some(cow_tx.transaction_body.raw_cbor().to_vec());
            let witness_cbor = Some(cow_tx.transaction_witness_set.raw_cbor().to_vec());
            (dugite_primitives::era::Era::Conway, body_cbor, witness_cbor)
        }
        // MultiEraTx is #[non_exhaustive]; treat any future variant as Conway.
        _ => (dugite_primitives::era::Era::Conway, None, None),
    };
...
fn convert_input(input: &PallasInput) -> TransactionInput {
    TransactionInput {
        transaction_id: pallas_hash_to_dugite32(input.hash()),
...
fn convert_output_inner(
    ...
    let datum = match output.datum() {
        Some(pallas_primitives::conway::DatumOption::Hash(h)) => {
            OutputDatum::DatumHash(pallas_hash_to_dugite32(&h))
        }
...
fn convert_native_script_inner(script: &pallas_primitives::alonzo::NativeScript) -> NativeScript {
    ...
    match script {
        PNS::ScriptPubkey(h) => {
            ...
            NativeScript::ScriptPubkey(pallas_hash_to_dugite28(h).to_hash32_padded())
        }
...
/// Convert a pallas Hash<32> to a dugite Hash32
pub fn pallas_hash_to_dugite32(hash: &pallas_crypto::hash::Hash<32>) -> Hash32 {
    let bytes: &[u8; 32] = hash;
    Hash::from_bytes(*bytes)
}

/// Convert a pallas Hash<28> to a dugite Hash28
pub fn pallas_hash_to_dugite28(hash: &pallas_crypto::hash::Hash<28>) -> Hash28 {
    let bytes: &[u8; 28] = hash;
    Hash::from_bytes(*bytes)
}

/// Convert a dugite Hash32 to a pallas Hash<32>
pub fn dugite_hash_to_pallas32(hash: &Hash32) -> pallas_crypto::hash::Hash<32> {
    pallas_crypto::hash::Hash::from(*hash.as_bytes())
}

/// Convert a dugite Hash28 to a pallas Hash<28>
pub fn dugite_hash_to_pallas28(hash: &Hash28) -> pallas_crypto::hash::Hash<28> {
    pallas_crypto::hash::Hash::from(*hash.as_bytes())
}
...
fn convert_pallas_stake_credential(cred: &pallas_primitives::StakeCredential) -> Credential {
    match cred {
        pallas_primitives::StakeCredential::AddrKeyhash(h) => {
            Credential::VerificationKey(pallas_hash_to_dugite28(h))
        }
        pallas_primitives::StakeCredential::ScriptHash(h) => {
            Credential::Script(pallas_hash_to_dugite28(h))
        }
    }
}
...
fn convert_alonzo_certificate(
    ...
    match cert {
        ...
        AC::StakeDelegation(cred, pool_hash) => Some(Certificate::StakeDelegation {
            ...
            pool_hash: pallas_hash_to_dugite28(pool_hash),
        }),
        AC::PoolRegistration {
            operator,
            vrf_keyhash,
            ...
            let owners = pool_owners.iter().map(pallas_hash_to_dugite28).collect();
            ...
            Some(Certificate::PoolRegistration(PoolParams {
                operator: pallas_hash_to_dugite28(operator),
                vrf_keyhash: pallas_hash_to_dugite32(vrf_keyhash),
...
fn convert_pallas_drep(drep: &pallas_primitives::conway::DRep) -> DRep {
    ...
    match drep {
        PD::Key(h) => {
            ...
            DRep::KeyHash(pallas_hash_to_dugite28(h).to_hash32_padded())
        }
        PD::Script(h) => DRep::ScriptHash(pallas_hash_to_dugite28(h)),
...
fn convert_pallas_anchor(anchor: &pallas_primitives::conway::Anchor) -> Anchor {
    Anchor {
        ...
        data_hash: pallas_hash_to_dugite32(&anchor.content_hash),
    }
}
            ...
            for (pallas_voter, votes_by_action) in voting_procs.iter() {
                ...
                for (pallas_action_id, pallas_proc) in votes_by_action.iter() {
                    let action_id = GovActionId {
                        transaction_id: pallas_hash_to_dugite32(&pallas_action_id.transaction_id),
...
fn convert_pallas_voter(voter: &pallas_primitives::conway::Voter) -> Voter {
    ...
    match voter {
        PV::ConstitutionalCommitteeKey(h) => {
            Voter::ConstitutionalCommittee(Credential::VerificationKey(pallas_hash_to_dugite28(h)))
        }
        PV::ConstitutionalCommitteeScript(h) => {
            Voter::ConstitutionalCommittee(Credential::Script(pallas_hash_to_dugite28(h)))
        }
        PV::DRepKey(h) => Voter::DRep(Credential::VerificationKey(pallas_hash_to_dugite28(h))),
        PV::DRepScript(h) => Voter::DRep(Credential::Script(pallas_hash_to_dugite28(h))),
        PV::StakePoolKey(h) => {
            // Pool key hash is 28 bytes; pad to Hash32
            Voter::StakePool(pallas_hash_to_dugite28(h).to_hash32_padded())
        }
    }
}
...
fn convert_pallas_gov_action(action: &pallas_primitives::conway::GovAction) -> GovAction {
    ...
    let convert_prev = |prev_id: &Option<pallas_primitives::conway::GovActionId>| {
        prev_id.as_ref().map(|id| GovActionId {
            transaction_id: pallas_hash_to_dugite32(&id.transaction_id),
    ...
    match action {
        PGA::ParameterChange(prev_id, update, script) => GovAction::ParameterChange {
            ...
            policy_hash: script.as_ref().map(pallas_hash_to_dugite28),
        },
        ...
        PGA::NewConstitution(prev_id, constitution) => GovAction::NewConstitution {
            ...
            constitution: Constitution {
                ...
                script_hash: constitution
                    .guardrail_script
                    .map(|h| pallas_hash_to_dugite28(&h)),
            },
        },
...
mod tests {
    ...
    fn test_hash32_conversion_roundtrip() {
        let dugite_hash = blake2b_256(b"test data");
        let pallas_hash = dugite_hash_to_pallas32(&dugite_hash);
        let back = pallas_hash_to_dugite32(&pallas_hash);
        assert_eq!(dugite_hash, back);
    }

    #[test]
    fn test_hash28_conversion_roundtrip() {
        let dugite_hash = Hash28::from_bytes([42u8; 28]);
        let pallas_hash = dugite_hash_to_pallas28(&dugite_hash);
        let back = pallas_hash_to_dugite28(&pallas_hash);
        assert_eq!(dugite_hash, back);
    }
