...
use dugite_serialization::cbor::*;
use dugite_serialization::encode::*;
use dugite_serialization::{decode_transaction, SerializationError};
...
fn test_encode_point_max_slot() {
    let point = dugite_primitives::block::Point::Specific(SlotNo(u64::MAX), Hash32::ZERO);
...
fn test_encode_point_slot_zero() {
    let point = dugite_primitives::block::Point::Specific(SlotNo(0), Hash32::ZERO);
    let enc = encode_point(&point);
