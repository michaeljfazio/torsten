...
use dugite_crypto::keys::TextEnvelope;
...
impl BlockProducerCredentials {
    ...
    pub fn load(vrf_skey_path: &Path, kes_skey_path: &Path, opcert_path: &Path) -> Result<Self> {
        // Load VRF signing key via text envelope.
        let vrf_env = TextEnvelope::from_file(vrf_skey_path)
            .with_context(|| format!("Failed to read VRF skey: {}", vrf_skey_path.display()))?;
        vrf_env
            .expect_type_one_of(&["VrfSigningKey_PraosVRF", "VrfSigningKey_ed25519_bip32"])
            .with_context(|| {
                format!(
                    "VRF skey file has wrong type: {} (expected VrfSigningKey_PraosVRF)",
                    vrf_env.type_
                )
            })?;
        let vrf_key_bytes = vrf_env.raw_cbor_bytes()?;
        ...
        let vrf_keypair = dugite_crypto::vrf::generate_vrf_keypair_from_secret(&vrf_skey);
        let vrf_vkey = vrf_keypair.public_key;

        // Load KES signing key via text envelope.
        let kes_env = TextEnvelope::from_file(kes_skey_path)
            .with_context(|| format!("Failed to read KES skey: {}", kes_skey_path.display()))?;
        kes_env
            .expect_type("KesSigningKey_ed25519_kes_2^6")
            .with_context(|| {
                format!(
                    "KES skey file has wrong type: {} (expected KesSigningKey_ed25519_kes_2^6)",
                    kes_env.type_
                )
            })?;
        let raw_kes_bytes = kes_env.raw_cbor_bytes()?;
        ...
        // previous Dugite session that persisted the buffer) we accept it as-is.
        let kes_key_bytes = match raw_kes_bytes.len() {
            ...
            dugite_crypto::kes::KES_SECRET_KEY_BUFFER_SIZE => {
                // Already has the period counter (written by Dugite itself).
                raw_kes_bytes
            }
            n => anyhow::bail!(
        ...
        // Load operational certificate via text envelope.
        // Opcert CBOR format: ([kes_vkey, sequence, kes_period, sigma], cold_vkey)
        let opcert_env = TextEnvelope::from_file(opcert_path)
            .with_context(|| format!("Failed to read opcert: {}", opcert_path.display()))?;
        opcert_env
            .expect_type_one_of(&["NodeOperationalCertificate", "OperationalCertificate"])
            .with_context(|| {
                format!(
                    "opcert file has wrong type: {} (expected NodeOperationalCertificate)",
                    opcert_env.type_
                )
            })?;
        let opcert_cbor = hex::decode(&opcert_env.cbor_hex)?;
        ...
        let (kes_vkey_bytes, opcert_sequence, opcert_kes_period, opcert_sigma, cold_vkey) =
            if outer_len == Some(2) {
                ...
                let cold = if outer_len == Some(5) {
                    ...
                    let cold_env = TextEnvelope::from_file(&cold_vkey_path).with_context(|| {
                        format!(
                            "Opcert has no cold_vkey; failed to load {}",
                            cold_vkey_path.display()
                        )
                    })?;
                    cold_env.raw_cbor_bytes()?
                };
                (kes_vkey, seq, kes_period, sigma, cold)
            };
...
///
/// Wraps `data` in the smallest valid CBOR byte string header.
/// Used by tests to construct synthetic key envelopes.
#[cfg(test)]
...
pub fn forge_block(
    ...
    let vrf_seed = dugite_consensus::slot_leader::vrf_input(epoch_nonce, slot);
    let (vrf_proof, vrf_output) =
        dugite_crypto::vrf::generate_vrf_proof(&creds.vrf_skey, &vrf_seed)
    ...
    let kes_signature = if !creds.kes_skey.is_empty() {
        let evolved_kes =
            dugite_crypto::kes::kes_evolve_to_period(&creds.kes_skey, kes_period_offset as u32)
                .map_err(|e| anyhow::anyhow!("KES key evolution failed: {e}"))?;

        let (sig_bytes, period) =
            dugite_crypto::kes::kes_sign_bytes(&evolved_kes, &header_body_cbor)
    ...
    let full_header_cbor = dugite_serialization::encode_block_header(&header, &kes_signature);
    ...
    let block_cbor = dugite_serialization::encode_block(&block, &kes_signature);
...
pub fn check_slot_leadership(
    ...
    match dugite_crypto::vrf::generate_vrf_proof(&creds.vrf_skey, &vrf_seed) {
        Ok((_proof, output)) => dugite_consensus::slot_leader::is_slot_leader_rational(
            &output,
            pool_stake,
            total_active_stake,
            f_num,
...
mod tests {
    use super::*;
    use dugite_primitives::transaction::TransactionBody;
    use std::collections::BTreeMap;

    fn make_test_credentials() -> BlockProducerCredentials {
        let vrf_kp = dugite_crypto::vrf::generate_vrf_keypair();
    ...
    fn test_forge_block_with_transactions() {
        ...
        let tx = Transaction {
            ...
            body: TransactionBody {
                inputs: vec![dugite_primitives::transaction::TransactionInput {
                    transaction_id: Hash32::ZERO,
                    index: 0,
                }],
                outputs: vec![dugite_primitives::transaction::TransactionOutput {
                ...
                fee: dugite_primitives::value::Lovelace(200_000),
                ttl: None,
                certificates: vec![],
    ...
    fn load_vrf_skey_from_raw(raw_vrf: &[u8]) -> anyhow::Result<[u8; 32]> {
        ...
        let seed = [0xABu8; 32];
        let (kes_sk_bytes, kes_pk_bytes) = dugite_crypto::kes::kes_keygen(&seed).unwrap();
    ...
    fn test_vrf_proof_same_from_32_or_64_byte_key() {
        ...
        let (proof_32, output_32) =
            dugite_crypto::vrf::generate_vrf_proof(&seed_32, &input).unwrap();
        let (proof_64, output_64) =
            dugite_crypto::vrf::generate_vrf_proof(&seed_64, &input).unwrap();
    ...
    fn load_creds_with_kes(raw_kes: &[u8]) -> anyhow::Result<BlockProducerCredentials> {
        ...
        let seed = [0x11u8; 32];
        let (_kes_sk, kes_pk_bytes) = dugite_crypto::kes::kes_keygen(&seed).unwrap();
        let kes_vkey: [u8; 32] = kes_pk_bytes[..32].try_into().unwrap();
    ...
    fn test_load_kes_skey_608_bytes_appends_period_counter() {
        let seed = [0x22u8; 32];
        let (kes_sk_612, _) = dugite_crypto::kes::kes_keygen(&seed).unwrap();
        assert_eq!(
            kes_sk_612.len(),
            dugite_crypto::kes::KES_SECRET_KEY_BUFFER_SIZE
        );
        ...
        let kes_sk_608 = &kes_sk_612[..dugite_crypto::kes::KES_SECRET_KEY_SIZE];
        ...
        let period =
            dugite_crypto::kes::kes_get_period(&creds.kes_skey).expect("kes_get_period must work");
        assert_eq!(
            period, 0,
    ...
    /// A key file that already has 612 bytes (e.g. written by a prior Dugite
    ...
    fn test_forge_block_with_608_byte_kes_key() {
        ...
        let seed = [0x44u8; 32];
        let (kes_sk_612, kes_pk_bytes) = dugite_crypto::kes::kes_keygen(&seed).unwrap();
        let kes_sk_608 = kes_sk_612[..dugite_crypto::kes::KES_SECRET_KEY_SIZE].to_vec();
