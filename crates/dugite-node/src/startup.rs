...
//!
//! This module implements the Dugite equivalent of Haskell's `openDBInternal`
...
use dugite_ledger::{ledger_seq::LedgerSeq, BlockValidationMode, LedgerState};
...
pub enum StartupError {
    ...
    Storage(#[from] dugite_storage::chain_db::ChainDBError),
...
fn replay_immutable_gap(
    ...
    for cbor in &blocks {
        let block =
            dugite_serialization::decode_block(cbor).map_err(|e| StartupError::BlockDecode {
...
mod tests {
    ...
    use dugite_primitives::protocol_params::ProtocolParameters;
    use dugite_primitives::time::SlotNo;
    use dugite_storage::chain_db::DEFAULT_SECURITY_PARAM_K;
