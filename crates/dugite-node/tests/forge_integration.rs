...
use dugite_consensus::{OuroborosPraos, ValidationMode};
use dugite_crypto::{kes, vrf};
use dugite_node::forge::{forge_block, BlockProducerConfig, BlockProducerCredentials};
...
/// `validate_operational_cert` unit tests in `dugite-consensus`).
fn synthetic_credentials() -> BlockProducerCredentials {
    ...
    let pool_id = dugite_primitives::hash::blake2b_224(&cold_vkey);
...
/// `dugite_crypto::vrf::generate_vrf_proof`; we then re-derive the expected
...
fn test_vrf_proof_valid_for_forged_block() {
    ...
    let expected_seed = dugite_consensus::slot_leader::vrf_input(&epoch_nonce, slot);
...
fn test_forged_block_header_hash_stable() {
    ...
    let full_header_cbor =
        dugite_serialization::encode_block_header(&block_a.header, &block_a.header.kes_signature);
...
fn test_kes_signature_verifies() {
    ...
    let header_body = dugite_serialization::encode_block_header_body(&block.header);
...
fn test_opcert_counter_tracking() {
    use dugite_consensus::praos::{BlockIssuerInfo, ConsensusError};
