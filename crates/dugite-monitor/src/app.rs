...
pub enum SyncState {
    ...
    ///
    /// During this phase `dugite_blocks_applied_total` remains 0 (blocks are
    /// replayed internally, not received from peers), while
    /// `dugite_sync_progress_percent` advances steadily.  This is distinct
...
/// Network name derived from the `dugite_network_magic` metric.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Network {
    Mainnet,
    Preview,
...
impl RttBands {
    ...
    pub fn from_snapshot(snap: &MetricsSnapshot) -> Self {
        ...
        let le1000 = snap
            ...
            .copied()
            .unwrap_or(0.0) as u64;
        let total = snap.get_u64("dugite_peer_handshake_rtt_ms_count");
        let sum = snap.get("dugite_peer_handshake_rtt_ms_sum");
        ...
        let gauge_avg = snap.values.get("dugite_peer_rtt_avg_ms").copied();
        let gauge_min = snap.values.get("dugite_peer_rtt_min_ms").copied();
        let gauge_max = snap.values.get("dugite_peer_rtt_max_ms").copied();
        ...
        let min_ms = match gauge_min {
            ...
            _ => snap
                .values
                .get("dugite_peer_handshake_rtt_ms_min")
                .copied()
                .or(min_approx),
        };
        let max_ms = match gauge_max {
            ...
            _ => snap
                .values
                .get("dugite_peer_handshake_rtt_ms_max")
                .copied()
                .or(max_approx),
        };
...
pub struct App {
    ...
    /// Network inferred from `dugite_network_magic`.
    pub network: Network,
    /// Epoch slot position.
    ...
    /// Ring-buffer of `dugite_blocks_applied` counter samples (one per poll).
    ///
...
impl App {
    ...
    pub fn update_metrics(&mut self, snapshot: MetricsSnapshot) {
        ...
        let blocks_applied = snapshot.get_u64("dugite_blocks_applied_total");
        ...
        // granularity.
        let cpu_pct = snapshot.get("dugite_cpu_percent");
    ...
    pub fn sync_status(&self) -> SyncState {
        ...
        let pct_raw = self.metrics.get("dugite_sync_progress_percent");
        ...
        let blocks_applied = self.metrics.get_u64("dugite_blocks_applied_total");
...
mod tests {
    ...
    fn test_epoch_progress_update() {
        ...
        app.update_metrics(make_snapshot(vec![
            ("dugite_slot_number", 43_200.0),
            ("dugite_network_magic", 2.0), // Preview: 86,400 slots/epoch
        ]));
    ...
    fn test_epoch_length_override() {
        ...
        app.update_metrics(make_snapshot(vec![("dugite_slot_number", 43_200.0)]));
    ...
    fn test_is_block_producer() {
        let mut app = App::new();
        app.metrics = make_snapshot(vec![("dugite_is_block_producer", 1.0)]);
        assert!(app.is_block_producer());

        app.metrics = make_snapshot(vec![("dugite_is_block_producer", 0.0)]);
        assert!(!app.is_block_producer());
    }
    ...
    fn test_sync_status() {
        ...
        app.metrics = make_snapshot(vec![
            ...
            ("dugite_tip_age_seconds", 100.0),
            ("dugite_blocks_applied_total", 250_000.0),
        ]);
    ...
    fn test_sync_status_replaying() {
        ...
        app.metrics = make_snapshot(vec![
            ("dugite_sync_progress_percent", 3500.0),
            ("dugite_tip_age_seconds", 86_400.0),
            ("dugite_blocks_applied_total", 0.0),
        ]);
        ...
        app.metrics = make_snapshot(vec![
            ("dugite_sync_progress_percent", 0.0),
            ("dugite_tip_age_seconds", 86_400.0),
            ("dugite_blocks_applied_total", 0.0),
        ]);
        ...
        app.metrics = make_snapshot(vec![
            ...
            ("dugite_tip_age_seconds", 600.0),
            ("dugite_blocks_applied_total", 1.0),
        ]);
        assert_eq!(app.sync_status(), SyncState::Stalled);
    }
    ...
    fn test_offline_preserves_last_known_values() {
        ...
        app.update_metrics(make_snapshot(vec![
            ("dugite_block_number", 4_000_000.0),
            ("dugite_slot_number", 100_000.0),
        ]));
        assert_eq!(app.node_status, NodeStatus::Online);
        assert_eq!(app.metrics.get_u64("dugite_block_number"), 4_000_000);
        ...
        assert_eq!(
            app.metrics.get_u64("dugite_block_number"),
            4_000_000,
    ...
    fn test_offline_then_online_clears_stale_state() {
        ...
        app.update_metrics(make_snapshot(vec![("dugite_block_number", 100.0)]));
        // Go offline.
        ...
        app.update_metrics(make_snapshot(vec![("dugite_block_number", 200.0)]));
        assert_eq!(app.node_status, NodeStatus::Online);
        assert!(
        ...
        assert!(!app.is_stale());
        assert!(app.metrics.connected);
        assert_eq!(app.metrics.get_u64("dugite_block_number"), 200);
    }
    ...
    fn test_offline_duration_is_none_when_online() {
        let mut app = App::new();
        app.update_metrics(make_snapshot(vec![("dugite_block_number", 1.0)]));
    ...
    fn test_block_rate_history_accumulates_deltas() {
        ...
        app.update_metrics(make_snapshot(vec![("dugite_blocks_applied_total", 1000.0)]));
        assert!(
        ...
        app.update_metrics(make_snapshot(vec![("dugite_blocks_applied_total", 1050.0)]));
        assert_eq!(app.block_rate_history.len(), 1);
        assert_eq!(app.block_rate_history[0], 50);

        // Third poll: 30 more.
        app.update_metrics(make_snapshot(vec![("dugite_blocks_applied_total", 1080.0)]));
    ...
    fn test_block_rate_history_capped_at_max_len() {
        ...
        app.update_metrics(make_snapshot(vec![("dugite_blocks_applied_total", 0.0)]));
    ...
    fn test_cpu_pct_history_accumulates_samples() {
        ...
        app.update_metrics(make_snapshot(vec![("dugite_cpu_percent", 25.0)]));
        assert_eq!(app.cpu_pct_history.len(), 1);
        // 25.0 % × 10 = 250.
        assert_eq!(app.cpu_pct_history[0], 250);

        app.update_metrics(make_snapshot(vec![("dugite_cpu_percent", 75.5)]));
        assert_eq!(app.cpu_pct_history.len(), 2);
        // 75.5 % × 10 rounded = 755.
        assert_eq!(app.cpu_pct_history[1], 755);

        // Zero CPU is a valid sample (idle node).
        app.update_metrics(make_snapshot(vec![("dugite_cpu_percent", 0.0)]));
    ...
    fn test_cpu_pct_history_capped_at_max_len() {
        ...
        for _ in 0..=(CPU_HISTORY_LEN + 5) {
            app.update_metrics(make_snapshot(vec![("dugite_cpu_percent", 50.0)]));
        }
        assert_eq!(
    ...
    fn test_cpu_pct_history_not_updated_while_offline() {
        ...
        app.update_metrics(make_snapshot(vec![("dugite_cpu_percent", 30.0)]));
        assert_eq!(app.cpu_pct_history.len(), 1);
