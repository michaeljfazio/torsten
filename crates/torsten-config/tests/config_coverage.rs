//! Integration tests for torsten-config CLI subcommands and config operations.

// Integration tests for torsten-config binary.
...
fn test_init_preview_generates_valid_json() {
    ...
    let output = std::process::Command::new(env!("CARGO_BIN_EXE_torsten-config"))
    ...
    match output {
        ...
        Ok(o) => {
            ...
            if stderr.contains("unrecognized") {
                eprintln!("Skipping test: torsten-config init not supported with these args");
                return;
            }
            panic!("init failed: {}", stderr);
        }
        Err(e) => {
            eprintln!("Skipping test: could not run torsten-config: {e}");
        }
    }
}
...
// Note: torsten-config is a binary crate with no lib target.
