...
use dugite_primitives::hash::{Hash28, PolicyId};
...
fn collect_plutus_script_hashes(tx: &Transaction, utxo_set: &dyn UtxoLookup) -> HashSet<Hash28> {
    ...
    for s in &tx.witness_set.plutus_v1_scripts {
        hashes.insert(dugite_primitives::hash::blake2b_224_tagged(1, s));
    }
    for s in &tx.witness_set.plutus_v2_scripts {
        hashes.insert(dugite_primitives::hash::blake2b_224_tagged(2, s));
    }
    for s in &tx.witness_set.plutus_v3_scripts {
        hashes.insert(dugite_primitives::hash::blake2b_224_tagged(3, s));
    }
...
pub(crate) fn plutus_script_version_map(
    ...
    for s in &tx.witness_set.plutus_v1_scripts {
        map.insert(dugite_primitives::hash::blake2b_224_tagged(1, s), 1);
    }
    for s in &tx.witness_set.plutus_v2_scripts {
        map.insert(dugite_primitives::hash::blake2b_224_tagged(2, s), 2);
    }
    for s in &tx.witness_set.plutus_v3_scripts {
        map.insert(dugite_primitives::hash::blake2b_224_tagged(3, s), 3);
    }
    for inp in tx.body.inputs.iter().chain(tx.body.reference_inputs.iter()) {
        if let Some(utxo) = utxo_set.lookup(inp) {
            if let Some(script_ref) = &utxo.script_ref {
