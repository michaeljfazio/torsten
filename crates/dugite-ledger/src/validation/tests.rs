...
mod tests {
    ...
    fn test_stake_registration_deposit() {
        ...
        tx.body.certificates.push(Certificate::StakeRegistration(
            dugite_primitives::credentials::Credential::VerificationKey(
                dugite_primitives::hash::Hash28::from_bytes([5u8; 28]),
            ),
        ));
        let result = validate_transaction(&tx, &utxo_set, &params, 100, 300, None);
        assert!(result.is_ok());
    ...
    fn test_stake_deregistration_refund() {
        ...
        let sk = dugite_crypto::keys::PaymentSigningKey::from_bytes(&[5u8; 32]).unwrap();
        let vk = sk.verification_key();
        let cred_hash = vk.hash();
        let mut tx = make_simple_tx(input, 10_000_000 - 200_000 + key_deposit, 200_000);
        tx.body.certificates.push(Certificate::StakeDeregistration(
            dugite_primitives::credentials::Credential::VerificationKey(cred_hash),
        ));
    ...
    fn test_deposits_and_refunds_uses_stored_deposit() {
        ...
        let cred = dugite_primitives::credentials::Credential::VerificationKey(
            dugite_primitives::hash::Hash28::from_bytes([7u8; 28]),
        );
    ...
    fn test_deposits_and_refunds_fallback_without_stored_deposit() {
        ...
        let cred = dugite_primitives::credentials::Credential::VerificationKey(
            dugite_primitives::hash::Hash28::from_bytes([8u8; 28]),
        );
        let certs = vec![Certificate::StakeDeregistration(cred)];
    ...
    fn test_multi_asset_with_minting() {
        ...
        let policy = dugite_primitives::hash::blake2b_224(&tagged);
        let asset = AssetName::new(b"Token".to_vec()).unwrap();
        let (utxo_set, input) = make_simple_utxo_set();
        let mut output_value = Value::lovelace(9_800_000);
        output_value
        ...
        let mut mint: BTreeMap<dugite_primitives::hash::PolicyId, BTreeMap<AssetName, i64>> =
            BTreeMap::new();
    ...
    fn make_plutus_tx_with_collateral(
        ...
        let params = ProtocolParameters::mainnet_defaults();
        let computed_hash = dugite_serialization::compute_script_data_hash(
    ...
    fn make_pool_owner_witness_for_tx(tx_hash: &Hash32) -> (VKeyWitness, Hash28) {
        let sk = dugite_crypto::keys::PaymentSigningKey::from_bytes(&POOL_OWNER_SEED).unwrap();
        let vk = sk.verification_key();
    ...
    fn test_pool_reregistration_no_duplicate_deposit() {
        let pool_id = dugite_primitives::hash::Hash28::from_bytes([42u8; 28]);
        let mut utxo_set = UtxoSet::new();
        let input = TransactionInput {
            transaction_id: Hash32::from_bytes([1u8; 32]),
            index: 0,
    ...
    fn test_witness_completeness_vkey_input_with_matching_witness() {
        ...
        let keyhash = dugite_primitives::hash::blake2b_224(&vkey_bytes);
        let mut utxo_set = UtxoSet::new();
        let input = TransactionInput {
            transaction_id: Hash32::from_bytes([1u8; 32]),
            index: 0,
        ...
        utxo_set.insert(
            ...
            TransactionOutput {
                address: Address::Enterprise(dugite_primitives::address::EnterpriseAddress {
                    network: dugite_primitives::network::NetworkId::Testnet,
                    payment: dugite_primitives::credentials::Credential::VerificationKey(keyhash),
                }),
                value: Value::lovelace(10_000_000),
                datum: OutputDatum::None,
                script_ref: None,
    ...
    fn test_extract_reward_credential_vkey() {
        ...
        assert_eq!(
            cred,
            Some(dugite_primitives::credentials::Credential::VerificationKey(
                keyhash
            ))
        );
    }
    ...
    fn test_extract_reward_credential_script() {
        ...
        assert_eq!(
            cred,
            Some(dugite_primitives::credentials::Credential::Script(
                script_hash
            ))
        );
    }
    ...
    fn test_conway_only_certificate_name_classification() {
        let conway_certs = vec![
            ...
            Certificate::CommitteeHotAuth {
                cold_credential: dugite_primitives::credentials::Credential::VerificationKey(
                    Hash28::from_bytes([0u8; 28]),
                ),
                hot_credential: dugite_primitives::credentials::Credential::VerificationKey(
    ...
    /// accepted it; Dugite incorrectly computed minimum = 168,581 (delta = 44 =
    /// 1 byte × min_fee_a(44)).  The root cause: Dugite included the `is_valid`
    ...
    ///   Dugite-OLD: 44 × 3830 + 155381 = 168,520 + 155,381 = (irrelevant - just verify delta)
    ///   Dugite-NEW: 44 × 3829 + 155381 = 168,476 + 155,381 = (irrelevant)
    ...
    fn test_within_block_ref_script_for_minting_resolution() {
        ...
        let mut mint_map: BTreeMap<dugite_primitives::hash::PolicyId, BTreeMap<AssetName, i64>> =
            BTreeMap::new();
        mint_map
            .entry(script_hash)
            .or_default()
    ...
    fn test_minting_policy_missing_mint_redeemer() {
        ...
        let policy_id = dugite_primitives::hash::blake2b_224_tagged(2, &script_bytes);
    ...
    fn test_mint_redeemer_sorted_index_order() {
        ...
        let policy_a = dugite_primitives::hash::blake2b_224_tagged(2, &script_a);
        let policy_b = dugite_primitives::hash::blake2b_224_tagged(2, &script_b);
    ...
    fn test_plutus_script_version_map_v3_detection() {
        ...
        let hash_v1 = dugite_primitives::hash::blake2b_224_tagged(1, &script_v1);
        let hash_v2 = dugite_primitives::hash::blake2b_224_tagged(2, &script_v2);
        let hash_v3 = dugite_primitives::hash::blake2b_224_tagged(3, &script_v3);
    ...
    fn test_propose_redeemer_parameter_change_with_policy_hash_missing() {
        use super::super::collateral::check_script_redeemers;
        use dugite_primitives::hash::ScriptHash;
        use dugite_primitives::transaction::{GovAction, ProposalProcedure, ProtocolParamUpdate};
    ...
    fn test_conway_stake_dereg_zero_balance_correct_refund_accepted() {
        ...
        let tx = make_dereg_tx(
            ...
            Certificate::ConwayStakeDeregistration {
                credential,
                refund: dugite_primitives::value::Lovelace(key_deposit),
            },
            key_deposit,
        );
        let reward_accounts = make_reward_accounts(cred_bytes, 0);
    ...
    fn test_conway_stake_dereg_wrong_refund_rejected() {
        ...
        let tx = make_dereg_tx(
            ...
            Certificate::ConwayStakeDeregistration {
                credential,
                refund: dugite_primitives::value::Lovelace(wrong_refund),
            },
            wrong_refund,
        );
        let reward_accounts = make_reward_accounts(cred_bytes, 0);
    ...
    fn test_conway_stake_reg_already_registered_rejected() {
        ...
        let tx = make_reg_tx(
            ...
            Certificate::ConwayStakeRegistration {
                credential,
                deposit: dugite_primitives::value::Lovelace(key_deposit),
            },
            key_deposit,
        );
    ...
    fn make_drep_reg_tx(utxo_set: &mut UtxoSet, cred_bytes: [u8; 28], deposit: u64) -> Transaction {
        ...
        tx.body.certificates.push(Certificate::RegDRep {
            credential,
            deposit: dugite_primitives::value::Lovelace(deposit),
            anchor: None,
        });
        tx
    }
    ...
    fn test_drep_reg_pre_conway_skips_check() {
        ...
        tx.body.certificates.push(Certificate::RegDRep {
            credential,
            deposit: dugite_primitives::value::Lovelace(drep_deposit),
            anchor: None,
        });
    ...
    fn test_aux_data_hash_content_mismatch() {
        ...
        let correct_hash = dugite_primitives::hash::blake2b_256(&raw_cbor);
    ...
    fn make_cert_vkey_witness(vkey: [u8; 32]) -> (VKeyWitness, Hash28) {
        let hash = dugite_primitives::hash::blake2b_224(&vkey);
    ...
    fn test_cert_witness_reg_drep_vkey_with_matching_witness() {
        ...
        tx.body.certificates.push(Certificate::RegDRep {
            credential: dugite_primitives::credentials::Credential::VerificationKey(keyhash),
    ...
    fn test_cert_witness_script_credential_no_vkey_required() {
        ...
        tx.body.certificates.push(Certificate::RegDRep {
            credential: dugite_primitives::credentials::Credential::Script(Hash28::from_bytes(
                [0xF1; 28],
            )),
            deposit: Lovelace(drep_deposit),
            anchor: None,
    ...
    fn test_cert_witness_multiple_certs_all_witnesses_present() {
        ...
        tx.body.certificates.push(Certificate::RegDRep {
            credential: dugite_primitives::credentials::Credential::VerificationKey(keyhash1),
            deposit: Lovelace(drep_deposit),
            anchor: None,
        });
        tx.body.certificates.push(Certificate::PoolRetirement {
