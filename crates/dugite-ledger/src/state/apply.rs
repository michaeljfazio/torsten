...
use dugite_primitives::transaction::{Certificate, TransactionInput};
    ...
    pub fn apply_block(
            ...
            && block.header.body_size > self.protocol_params.max_block_body_size
        {
            return Err(LedgerError::WrongBlockBodySize {
                ...
                claimed: self.protocol_params.max_block_body_size,
            });
        }
                ...
                if tx.is_valid {
                    ...
                    if let Err(errors) = result {
                        ...
                        if has_script_failure {
                            ...
                            // a protocol rule difference or a bug in Dugite.  These are
