...
pub fn is_slot_leader(vrf_output: &[u8], relative_stake: f64, active_slot_coeff: f64) -> bool {
    ...
    dugite_crypto::vrf::check_leader_value(&leader_value, relative_stake, active_slot_coeff)
}
...
pub fn is_slot_leader_rational(
    ...
    let leader_value = vrf_leader_value(vrf_output);
    dugite_crypto::vrf::check_leader_value_full_rational(
        &leader_value,
        sigma_num,
        sigma_den,
        f_num,
...
pub fn compute_leader_schedule(
    ...
    let active_slot_log = if !all_slots_active {
        match dugite_crypto::vrf::compute_active_slot_log(f_num, f_den) {
            Some(log) => log,
        ...
        match dugite_crypto::vrf::compute_active_slot_log(1, 2) {
            Some(log) => log,
            None => return Vec::new(),
        }
    };
    ...
    for offset in 0..epoch_length {
        ...
        if let Ok((proof, output)) = dugite_crypto::vrf::generate_vrf_proof(vrf_skey, &seed) {
            ...
            let is_leader = if all_slots_active {
                ...
                dugite_crypto::vrf::check_leader_value_full_rational_cached(
                    &leader_value,
                    pool_stake,
                    total_active_stake,
                    &active_slot_log,
...
mod tests {
    ...
    fn test_check_slot_leadership_1pct_stake() {
        ...
        assert!(
            dugite_crypto::vrf::check_leader_value(&zero_leader_value, 0.01, 0.05),
        ...
        assert!(
            !dugite_crypto::vrf::check_leader_value(&max_leader_value, 0.01, 0.05),
    ...
    fn test_check_slot_leadership_50pct_stake() {
        let zero_lv = [0u8; 32];
        assert!(
            dugite_crypto::vrf::check_leader_value(&zero_lv, 0.5, 0.05),
            "certNat=0 must always elect for 50% stake"
        );

        let high_lv = [0xFFu8; 32];
        assert!(
            !dugite_crypto::vrf::check_leader_value(&high_lv, 0.5, 0.05),
    ...
    fn test_check_slot_leadership_stake_monotonicity() {
        ...
        for output_byte in [0x00u8, 0x01, 0x04, 0x08, 0x10] {
            ...
            let results: Vec<bool> = stakes
                .iter()
                .map(|&s| dugite_crypto::vrf::check_leader_value(&leader_value, s, 0.05))
                .collect();
    ...
    fn test_check_slot_leadership_f_monotonicity() {
        ...
        let elected_f005 = dugite_crypto::vrf::check_leader_value(&test_lv, stake, 0.05);
        let elected_f010 = dugite_crypto::vrf::check_leader_value(&test_lv, stake, 0.10);
    ...
    fn test_check_slot_leadership_tpraos_zero_elected() {
        ...
        assert!(
            dugite_crypto::vrf::check_leader_value_tpraos(&zero_output, 1.0, 0.05),
            "TPraos: certNat=0 must always be elected with non-zero stake"
        );
        assert!(
            dugite_crypto::vrf::check_leader_value_tpraos(&zero_output, 0.01, 0.05),
    ...
    fn test_check_slot_leadership_tpraos_max_not_elected() {
        ...
        assert!(
            !dugite_crypto::vrf::check_leader_value_tpraos(&max_output, 1.0, 0.05),
    ...
    fn test_check_slot_leadership_tpraos_zero_stake() {
        let zero_output = [0u8; 64];
        assert!(
            !dugite_crypto::vrf::check_leader_value_tpraos(&zero_output, 0.0, 0.05),
    ...
    fn test_leader_schedule_deterministic() {
        ...
        let kp = dugite_crypto::vrf::generate_vrf_keypair_from_secret(&secret);
    ...
    fn test_leader_schedule_proof_verification() {
        ...
        for ls in &schedule {
            ...
            let verified =
                dugite_crypto::vrf::verify_vrf_proof(&kp.public_key, &ls.vrf_proof, &seed);
            assert!(
                verified.is_ok(),
    ...
    fn test_leader_schedule_different_keys_different_schedules() {
        let kp_a = dugite_crypto::vrf::generate_vrf_keypair_from_secret(&[0xAAu8; 32]);
        let kp_b = dugite_crypto::vrf::generate_vrf_keypair_from_secret(&[0xBBu8; 32]);
    ...
    fn test_leader_election_rate_full_stake() {
        ...
        for i in 0..trials {
            ...
            if dugite_crypto::vrf::check_leader_value(&lv, stake, f) {
                elected += 1;
            }
        }
    ...
    fn test_end_to_end_vrf_leader_pipeline() {
        ...
        let (proof, raw_output) = dugite_crypto::vrf::generate_vrf_proof(kp.secret_key(), &seed)
            .expect("VRF proof generation must succeed");

        // Step 3: verify VRF proof
        let verified_output = dugite_crypto::vrf::verify_vrf_proof(&kp.public_key, &proof, &seed)
        ...
        let from_check_leader = dugite_crypto::vrf::check_leader_value(&leader_value, 1.0, 0.05);
        assert_eq!(
    ...
    fn test_leader_schedule_consistent_with_is_slot_leader() {
        ...
        for offset in 0..epoch_len {
            ...
            let seed = vrf_input(&epoch_nonce, slot);
            if let Ok((_, output)) = dugite_crypto::vrf::generate_vrf_proof(kp.secret_key(), &seed)
            {
    ...
    fn test_check_slot_leadership_stake_above_one_no_panic() {
        ...
        let result = dugite_crypto::vrf::check_leader_value(&lv, 2.0, 0.05);
