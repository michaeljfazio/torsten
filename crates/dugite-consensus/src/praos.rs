...
impl OuroborosPraos {
    pub fn new() -> Self {
        ...
        OuroborosPraos {
            ...
            epoch_length: dugite_primitives::time::mainnet_epoch_length(),
        ...
        if let Some(info) = issuer_info {
            ...
            if header.vrf_result.output.len() == 64 {
                ...
                let is_leader = if is_praos {
                    ...
                    dugite_crypto::vrf::check_leader_value_tpraos_rational(
    ...
    fn verify_nonce_vrf_proof(&self, header: &BlockHeader) -> Result<(), ConsensusError> {
        ...
        match dugite_crypto::vrf::verify_vrf_proof(&header.vrf_vkey, &header.nonce_vrf_proof, &seed)
        {
            Ok(vrf_output) => {
    ...
    fn verify_kes_signature(&self, header: &BlockHeader) -> Result<(), ConsensusError> {
        ...
        match dugite_crypto::kes::kes_verify_bytes(
            &hot_vkey,
...
pub fn verify_leader_eligibility(
    ...
    if dugite_crypto::vrf::check_leader_value(vrf_output, relative_stake, active_slot_coeff) {
        Ok(())
    } else {
...
mod tests {
    ...
    fn test_verify_opcert_signature_wrong_key() {
        let cold_sk = dugite_crypto::keys::PaymentSigningKey::generate();
        let wrong_vk = dugite_crypto::keys::PaymentSigningKey::generate().verification_key();
    ...
    fn test_opcert_counter_same_value_ok() {
        ...
        let pool_id = dugite_primitives::hash::blake2b_224(&header1.issuer_vkey);
    ...
    fn test_opcert_counter_different_pools() {
        ...
        let pool_a = dugite_primitives::hash::blake2b_224(&header_a.issuer_vkey);
        let pool_b = dugite_primitives::hash::blake2b_224(&header_b.issuer_vkey);
    ...
    fn test_unregistered_pool_rejected_strict() {
        ...
        let header = make_valid_header(100);
        let expected_pool_id = dugite_primitives::hash::blake2b_224(&header.issuer_vkey);
    ...
    fn test_tpraos_nonce_vrf_verification_valid() {
        ...
        let kp = dugite_crypto::vrf::generate_vrf_keypair_from_secret(&vrf_skey);
        ...
        let (proof, output) =
            dugite_crypto::vrf::generate_vrf_proof(&vrf_skey, &nonce_seed).unwrap();
