use dugite_primitives::block::{BlockHeader, Point, Tip};
...
//
// Design note: `dugite-consensus` does NOT depend on `dugite-storage` (that
// would create a dependency cycle through `dugite-node`).  The
...
pub fn chain_preference(
    ...
    match (current_tip_header, candidate_tip_header) {
        (Some(cur), Some(cand)) => {
            ...
            // dugite_primitives::era directly (it's already available via the
...
/// `dugite-storage` crate, avoiding a circular dependency.  The concrete
/// implementation for `VolatileDB` lives in `dugite-storage` (or `dugite-node`)
...
pub trait SuccessorProvider {
    ...
    fn successors_of(
        &self,
        parent_hash: &dugite_primitives::hash::Hash32,
    ) -> Vec<dugite_primitives::hash::Hash32>;

    /// Return the `BlockHeader` for a volatile block, or `None` if unknown.
    fn header_of(
        &self,
        hash: &dugite_primitives::hash::Hash32,
...
pub fn maximal_candidates(
    immutable_tip: &dugite_primitives::block::Point,
    provider: &dyn SuccessorProvider,
    new_block_hash: &dugite_primitives::hash::Hash32,
) -> Vec<ChainFragment> {
    ...
    let prefix: Vec<dugite_primitives::block::BlockHeader> = {
    ...
    let mut leaf_suffixes: Vec<Vec<dugite_primitives::block::BlockHeader>> = Vec::new();

    // Stack: (current_hash, headers_accumulated_so_far_in_this_path)
    let mut stack: Vec<(Hash32, Vec<dugite_primitives::block::BlockHeader>)> = Vec::new();
    ...
    leaf_suffixes
        ...
        .map(|suffix| {
            ...
            // duplication.
            let mut all_headers: Vec<dugite_primitives::block::BlockHeader> = prefix.to_vec();
