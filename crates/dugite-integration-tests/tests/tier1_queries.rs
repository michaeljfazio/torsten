...
//! Run with: DUGITE_INTEGRATION_SOCKET=./node.sock cargo test -p dugite-integration-tests -- tier1

use dugite_integration_tests::helpers::cli::{integration_socket, run_cli, run_cli_ok};
...
fn tier1_query_utxo_fresh_address() {
    ...
    let keys = dugite_integration_tests::helpers::keys::TempKeys::new();
...
fn tier1_prometheus_metrics() {
    ...
    match result {
        Ok(output) if output.status.success() => {
            ...
            let expected_metrics = ["dugite_slot_number", "dugite_block_number"];
...
fn tier1_cross_verify_tip_with_koios() {
    ...
    let koios_tip = match rt.block_on(dugite_integration_tests::helpers::koios::tip()) {
        Ok(t) => t,
        Err(e) => {
