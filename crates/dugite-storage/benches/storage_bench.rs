    ...
    for &size in BLOCK_INDEX_SIZES {
        ...
        group.bench_with_input(BenchmarkId::new("in_memory", size), &size, |b, _| {
            b.iter(|| {
                ...
                let dir = tempfile::tempdir().unwrap();
                let mut idx =
                    dugite_storage::block_index::BlockIndex::new(&config, dir.path()).unwrap();
                for (i, hash) in hashes.iter().enumerate() {
                    idx.insert(
                        *hash,
                        dugite_storage::block_index::BlockLocation {
                            chunk_num: 0,
...
fn bench_block_index_lookup(c: &mut Criterion) {
    ...
    for &size in BLOCK_INDEX_SIZES {
        ...
        let mut idx_inmem =
            dugite_storage::block_index::BlockIndex::new(&config_inmem, dir_inmem.path()).unwrap();
        for (i, hash) in hashes.iter().enumerate() {
            idx_inmem.insert(
                *hash,
                dugite_storage::block_index::BlockLocation {
        ...
        let mut idx_mmap =
            dugite_storage::block_index::BlockIndex::new(&config_mmap, dir_mmap.path()).unwrap();
        for (i, hash) in hashes.iter().enumerate() {
            idx_mmap.insert(
                *hash,
                dugite_storage::block_index::BlockLocation {
...
fn bench_block_index_contains_miss(c: &mut Criterion) {
    ...
    let mut idx = dugite_storage::block_index::BlockIndex::new(&config, dir.path()).unwrap();
    for (i, hash) in hashes.iter().enumerate() {
        idx.insert(
            *hash,
            dugite_storage::block_index::BlockLocation {
    ...
    let mut idx2 = dugite_storage::block_index::BlockIndex::new(&config2, dir2.path()).unwrap();
    for (i, hash) in hashes.iter().enumerate() {
        idx2.insert(
            *hash,
            dugite_storage::block_index::BlockLocation {
