...
/// Grace period for newly connected warm peers before the governor requires
/// them to have served blocks before promoting to hot.
///
/// A brand-new connection has not yet had time to receive a ChainSync
/// header or serve a block fetch, so we allow `PROMOTION_NEW_PEER_GRACE`
/// before the liveness gate kicks in.  30 seconds is generous — typically
/// the first block should arrive within a few seconds of the handshake.
///
...
impl PeerManager {
    ...
    /// Total number of known peers (cold + warm + hot).
    ///
    /// Used by the governor to compare against `targets.known_peers` when
    /// deciding whether to request peer sharing.
    pub fn known_peer_count(&self) -> usize {
        self.peers.len()
    }

    /// Return addresses of warm and hot peers that are candidates for a peer
    /// sharing request, sorted by selection score (best first).
    ///
    /// Only outbound-connected peers are included — inbound peers may not have
    /// a publicly reachable listen address and therefore cannot be asked for
    /// peer sharing responses reliably.
    pub fn warm_hot_peer_addrs_for_sharing(&self) -> Vec<SocketAddr> {
        let mut candidates: Vec<(SocketAddr, f64)> = self
            .hot_peers
            .iter()
            ...
            .filter_map(|addr| {
                let info = self.peers.get(addr)?;
                // Only peers we dialled can be asked for sharing (see direction comment above)
                if info.direction != Some(ConnectionDirection::Outbound) {
                    return None;
                }
                Some((*addr, info.selection_score()))
            })
            .collect();
        // Best score first so the governor picks the most reliable peers first.
        candidates.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap_or(std::cmp::Ordering::Equal));
        candidates.into_iter().map(|(addr, _)| addr).collect()
    }

    /// Return cold peer addresses sorted for surplus eviction: non-config peers
    /// first, then ordered by (age, reputation) — oldest and worst-reputation
    /// first so the governor can trim the known set from the least-useful end.
    ///
    /// Config peers (topology file entries) are always excluded from eviction.
    pub fn cold_peers_for_eviction(&self) -> Vec<SocketAddr> {
        let now = Instant::now();
        let mut candidates: Vec<(SocketAddr, Duration, f64)> = self
            .cold_peers
            .iter()
            .filter_map(|addr| {
                let info = self.peers.get(addr)?;
                // Config-sourced peers are topology entries — never evict them.
                if info.source == PeerSource::Config {
                    return None;
                }
                // Age proxy: how long since last connection attempt (or infinity
                // if never connected — treat as maximally old).
                let age = info
                    .last_connected
                    .map(|t| now - t)
                    .unwrap_or(Duration::from_secs(u64::MAX / 2));
                Some((*addr, age, info.performance.reputation))
            })
            .collect();

        // Sort by age descending (oldest first), breaking ties by reputation
        // ascending (worst reputation first).  This evicts the peers that have
        // been idle the longest and contributed the least.
        candidates.sort_by(|a, b| {
            b.1.cmp(&a.1) // older first
                .then_with(|| {
                    a.2.partial_cmp(&b.2).unwrap_or(std::cmp::Ordering::Equal) // worse rep first
                })
        });
        candidates.into_iter().map(|(addr, _, _)| addr).collect()
    }
    ...
    /// Get full peer state for a peer address.
    ///
    /// Used by the governor for stall detection and error-rate checks.
    pub fn peer_info(&self, addr: &SocketAddr) -> Option<&PeerInfo> {
        self.peers.get(addr)
    }

    /// Return `true` if a warm peer is ready to be promoted to hot.
    ///
    /// A warm peer is considered promotion-ready if it satisfies at least one
    /// of the following conditions:
    ///
    /// 1. It has fetched at least one block since connecting (`blocks_fetched > 0`).
    ///    This is the primary liveness signal: the peer has proven it can serve
    ///    blocks and is not stalled immediately after the handshake.
    ///
    /// 2. It connected recently (within `PROMOTION_NEW_PEER_GRACE`) and has not
    ///    yet accumulated any failures.  Brand-new connections have not had time
    ///    to fetch blocks; refusing to promote them would penalise freshly
    ///    connected peers that are still in the process of establishing the
    ///    ChainSync protocol.
    ///
    /// 3. It has a recorded `last_good_fetch` — meaning it has served blocks in
    ///    the past, even if none since the current warm dwell period started.
    ///    This allows peers that are momentarily quiet (e.g. caught up at tip)
    ///    to still qualify for hot promotion.
    ///
    ...
    pub fn is_promotion_ready(&self, addr: &SocketAddr) -> bool {
        ...
        // Peers that have served blocks are clearly active.
        if perf.blocks_fetched > 0 {
            return true;
        }

        // Peers with a known-good fetch history (re-promoted peer).
        if perf.last_good_fetch.is_some() {
            return true;
        }

        // Peers with zero failures are eligible — this covers governor-managed
        // connections (TxSubmission2-only) that never serve blocks but are still
        // useful for tx propagation.
        if info.failure_count == 0 {
            return true;
        }
...
// ── Test-only helpers ────────────────────────────────────────────────────────
// These methods are exposed only in test builds so that integration tests in
// other modules (e.g. governor.rs) can set up precise peer states without
// having to reach into private struct fields.

#[cfg(test)]
impl PeerManager {
    /// Override the `last_connected` timestamp for a peer.
    ///
    /// Used by governor tests to simulate peers that connected a long time
    /// ago (past the promotion grace period) without having to `std::thread::sleep`.
    pub fn set_last_connected(&mut self, addr: &SocketAddr, when: Instant) {
        if let Some(info) = self.peers.get_mut(addr) {
            info.last_connected = Some(when);
        }
    }

    /// Override the `failure_count` for a peer.
    ///
    /// Used by governor tests to trigger the error-rate demotion path.
    pub fn set_failure_count(&mut self, addr: &SocketAddr, count: u32) {
        if let Some(info) = self.peers.get_mut(addr) {
            info.failure_count = count;
        }
    }
}
