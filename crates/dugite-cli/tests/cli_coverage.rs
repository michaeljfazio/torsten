//! Comprehensive test coverage for dugite-cli commands.
//!
...
fn test_keygen_produces_different_keys_each_time() {
    let sk1 = dugite_crypto::keys::PaymentSigningKey::generate();
    let sk2 = dugite_crypto::keys::PaymentSigningKey::generate();
...
fn test_kes_keygen_produces_valid_key_pair() {
    let seed = [42u8; 32];
    let result = dugite_crypto::kes::kes_keygen(&seed);
    assert!(result.is_ok());
...
fn test_vrf_keygen_different_each_time() {
    let kp1 = dugite_crypto::vrf::generate_vrf_keypair();
    let kp2 = dugite_crypto::vrf::generate_vrf_keypair();
...
fn test_vrf_keygen_sign_verify_roundtrip() {
    ...
    let (proof, output) = dugite_crypto::vrf::generate_vrf_proof(kp.secret_key(), seed).unwrap();
    assert_eq!(proof.len(), 80);
    assert_eq!(output.len(), 64);

    let verified = dugite_crypto::vrf::verify_vrf_proof(&kp.public_key, &proof, seed).unwrap();
...
fn test_kes_keygen_sign_verify_roundtrip() {
    ...
    let (sig, period) = dugite_crypto::kes::kes_sign_message(&sk_bytes, message).unwrap();
    assert_eq!(period, 0);
    assert!(dugite_crypto::kes::kes_verify(&pk_bytes, 0, &sig, message).is_ok());
}
...
fn test_vrf_key_envelope_cbor_format() {
    ...
    let (proof, _) = dugite_crypto::vrf::generate_vrf_proof(&sk_bytes, b"test").unwrap();
    assert_eq!(proof.len(), 80);
}
