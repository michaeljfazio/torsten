...
fn build_tx_body_cbor(
    ...
    if let Some(aux_data) = auxiliary_data {
        let aux_hash = dugite_primitives::hash::blake2b_256(aux_data);
        enc.u32(7)?;
...
fn parse_json_script_list(
    json: &serde_json::Value,
) -> Result<Vec<dugite_primitives::transaction::NativeScript>> {
...
async fn cmd_build(args: BuildArgs) -> Result<()> {
    ...
    let fee = if let Some(f) = explicit_fee {
        ...
        // Connect to the node.
        let mut client = dugite_network::N2CClient::connect(sock, magic)
            .await
            .map_err(|e| {
    ...
    pub fn run(self) -> Result<()> {
        match self.command {
            ...
            TxSubcommand::Sign {
                ...
                let tx_hash = dugite_crypto::signing::hash_transaction(&tx_body_cbor);
                ...
                for key_file in &signing_key_file {
                    ...
                    let sk = dugite_crypto::keys::PaymentSigningKey::from_bytes(key_bytes)?;
            ...
            TxSubcommand::Submit {
                ...
                let tx_hash = dugite_crypto::signing::hash_transaction(&body_cbor);

                // Submit via N2C socket
                let rt = tokio::runtime::Runtime::new()?;
                rt.block_on(async {
                    let magic = testnet_magic.unwrap_or(764824073);
                    let mut client = dugite_network::N2CClient::connect(&socket_path, magic)
                        .await
            ...
            TxSubcommand::Txid { tx_file } => {
                ...
                let hash = dugite_crypto::signing::hash_transaction(&body_cbor);
                println!("{hash}");
                Ok(())
            }
            ...
            TxSubcommand::Policyid { script_file } => {
                ...
                let script_cbor =
                    dugite_serialization::encode::encode_native_script(&native_script);
                ...
                let hash = dugite_primitives::hash::blake2b_224(&hash_input);
                println!("{}", hash.to_hex());
                Ok(())
            }
            TxSubcommand::HashScriptData {
                ...
                let hash = dugite_primitives::hash::blake2b_256(&preimage);
...
mod tests {
    ...
    fn test_parse_json_native_script_sig() {
        ...
        match script {
            dugite_primitives::transaction::NativeScript::ScriptPubkey(hash) => {
                let expected =
    ...
    fn test_parse_json_native_script_all() {
        ...
        let script = parse_json_native_script(&json).unwrap();
        match script {
            dugite_primitives::transaction::NativeScript::ScriptAll(scripts) => {
    ...
    fn test_parse_json_native_script_time_locks() {
        ...
        assert_eq!(
            script,
            dugite_primitives::transaction::NativeScript::InvalidBefore(
                dugite_primitives::time::SlotNo(500)
            )
        );
        ...
        assert_eq!(
            script,
            dugite_primitives::transaction::NativeScript::InvalidHereafter(
                dugite_primitives::time::SlotNo(999)
            )
        );
    }
    ...
    fn test_parse_json_native_script_at_least() {
        ...
        let script = parse_json_native_script(&json).unwrap();
        match script {
            dugite_primitives::transaction::NativeScript::ScriptNOfK(n, scripts) => {
                assert_eq!(n, 2);
    ...
    fn test_tx_body_has_script_data_hash_field_when_script_present() {
        ...
        let hash = dugite_primitives::hash::Hash32::from_bytes([0xab; 32]);
