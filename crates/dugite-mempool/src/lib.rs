...
/// Implement `MempoolProvider` from `dugite-primitives` for the concrete `Mempool`.
///
/// This allows `dugite-network` (and any other crate) to depend on the trait
/// abstraction rather than on this crate directly.
impl dugite_primitives::mempool::MempoolProvider for Mempool {
    fn add_tx(
        &self,
        tx_hash: TransactionHash,
        tx: Transaction,
        ...
        dugite_primitives::mempool::MempoolAddResult,
        dugite_primitives::mempool::MempoolAddError,
    > {
        match Mempool::add_tx(self, tx_hash, tx, size_bytes) {
            Ok(MempoolAddResult::Added) => Ok(dugite_primitives::mempool::MempoolAddResult::Added),
            Ok(MempoolAddResult::AlreadyExists) => {
                Ok(dugite_primitives::mempool::MempoolAddResult::AlreadyExists)
            }
            Err(e) => Err(dugite_primitives::mempool::MempoolAddError(e.to_string())),
        }
    }
    ...
    fn add_tx_with_fee(
        ...
        tx: Transaction,
        size_bytes: usize,
        fee: dugite_primitives::value::Lovelace,
    ) -> Result<
        dugite_primitives::mempool::MempoolAddResult,
        dugite_primitives::mempool::MempoolAddError,
    > {
    ...
    fn snapshot(&self) -> dugite_primitives::mempool::MempoolSnapshot {
        let snap = Mempool::snapshot(self);
        dugite_primitives::mempool::MempoolSnapshot {
...
mod tests {
    ...
    fn test_mempool_provider_trait_object() {
        use dugite_primitives::mempool::MempoolProvider;
        ...
        assert!(matches!(
            result,
            dugite_primitives::mempool::MempoolAddResult::Added
        ));
    ...
    fn test_mempool_provider_add_with_fee() {
        ...
        assert!(matches!(
            result,
            dugite_primitives::mempool::MempoolAddResult::AlreadyExists
        ));
    }
    ...
    // `validate_transaction` function from dugite-ledger (or focused parameter
    ...
    fn revalidate_with_params(
        ...
        mempool.revalidate_all(|tx| {
            ...
            dugite_ledger::validation::validate_transaction(tx, utxo, params, slot, tx_size, None)
                .is_ok()
        })
    }
