...
//!
//! Ported from the old torsten-network `query_handler` module. Contains the
...
// downstream consumers that previously imported from torsten_network::query_handler.
...
// ─── New torsten_network::QueryHandler trait implementation ─────────────────

/// Encode a `QueryResult` value to raw CBOR bytes (no MsgResult envelope, no HFC wrapper).
///
/// Used by the `torsten_network::QueryHandler` trait implementation to produce
...
impl torsten_network::QueryHandler for QueryHandler {
    ...
    fn validate_acquire(
        &self,
        _target: &torsten_network::protocol::local_state_query::AcquireTarget,
    ) -> Result<(), torsten_network::protocol::local_state_query::AcquireFailure> {
...
mod tests {
    ...
    /// Test the new torsten_network::QueryHandler trait implementation
    #[test]
    fn test_trait_handle_block_query() {
        use torsten_network::QueryHandler as TraitQueryHandler;
