...
### Crypto, Serialization & Primitives (`dugite-crypto`, `dugite-serialization`, `dugite-primitives`)
...
- **Helm Charts:** `charts/dugite/`, version must be bumped when templates change
- **CI/CD:** GitHub Actions (`.github/workflows/`), build/test/fmt/clippy
- **Monitoring:** Prometheus metrics on 12798, Grafana dashboards

### TUI (`dugite-monitor`, `dugite-config`)

**dugite-monitor:** ratatui-based dashboard, Prometheus polling + optional N2C socket queries, vim-style navigation
**dugite-config:** Interactive config editor with parameter schema, validation, documentation, diff view
...
2. Check integration points in `crates/dugite-node/src/`
...
You have a persistent, file-based memory system at `/Users/michaelfazio/Source/dugite/.claude/agent-memory/tech-lead/`. This directory may not exist yet — create it with mkdir if needed.
