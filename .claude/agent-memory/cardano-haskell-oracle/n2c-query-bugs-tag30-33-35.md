...
- Dugite sends: `IndividualPoolStake` format with tag(30) rationals and VRF hashes
...
- Dugite sends: `array(1)[0]` for Nothing — this is the Sum encoding, NOT Maybe encoding
- Fix: Change to `enc.array(0)` with no contents

## Tag 35: QueryStakePoolDefaultVote — Two bugs
1. **Argument**: Haskell sends single `KeyHash` (28 bytes), Dugite expects `tag(258) Set`
2. **Response**: Haskell returns bare `word8` (0/1/2), Dugite returns `Map<PoolId, [vote]>`
3. **Semantics**: DefaultNo=0, DefaultAbstain=1, DefaultNoConfidence=2
   - AlwaysAbstain → 1 (correct)
   - AlwaysNoConfidence → 2 (Dugite had 0, wrong)
   - Specific DRep/undelegated → 0 (Dugite had 2, wrong)
