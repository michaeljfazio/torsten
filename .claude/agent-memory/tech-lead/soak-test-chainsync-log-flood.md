...
description: Inbound Haskell node syncing from Torsten generates 1.2M+ "ChainSync at tip" INFO log messages per 10 minutes, creating I/O saturation that causes Torsten's own sync to stall after ~40 minutes
type: project
---

When a Haskell cardano-node connects to Torsten as an inbound peer and begins syncing the entire chain from scratch (e.g. from genesis), it generates a ChainSync server flood:

- Each header delivered by Torsten's ChainSync server logs: "ChainSync at tip — awaiting new blocks peer_addr=127.0.0.1:3002 headers_received=N"
- Rate: ~1.2M log messages per 10 minutes (~2000/sec)
- The log file fills at ~120MB/10min
- This I/O saturation causes Torsten's main sync loop to stall: no new "Chain extended" messages, stuck at same block for 10-15 minutes
...
**File to fix:** `crates/torsten-node/src/node/sync.rs` — find "ChainSync at tip" log and change INFO to DEBUG or add a counter modulo gate.
