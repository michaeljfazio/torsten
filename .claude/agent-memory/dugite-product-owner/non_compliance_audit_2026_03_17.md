...
description: Comprehensive audit of all known Cardano standards non-compliance in Dugite as of 2026-03-17
type: project
---

# Dugite Non-Compliance Audit — 2026-03-17
...
- **Dugite:** Computes Blake2b-256(CBOR_array(tx_bodies)) — single hash, wrong components
- **Crate:** dugite-node (forge.rs), dugite-serialization
...
- **Dugite:** Uses 4k/f unconditionally — wrong for Babbage and all TPraos eras (Shelley through Alonzo)
- **Crate:** dugite-consensus (praos.rs)
...
- **Dugite:** Computes pool distribution from `snapshots.set` on-the-fly per batch
- **Impact:** Leader schedule diverges from Haskell mid-epoch when stake changes
- **Crate:** dugite-consensus / dugite-ledger
...
- **Dugite:** Uses f64 — cannot exactly represent 0.05 (binary rounding error at boundary cases)
- **Crate:** dugite-consensus (praos.rs PoolInfo.relative_stake: f64)
...
- **Dugite:** Encodes [enacted_seq, expired_seq, delayed, future_pparams] — completely wrong structure; missing EnactState, has spurious future_pparams field
- **Issue:** #117
- **Crate:** dugite-network (n2c/query/encoding.rs:1314, encode_ratify_state)
...
- **Dugite (in GetGovState):** Fixed with correct field order as of ~2026-03-17 (encoding.rs:978-981 shows correct structure)
...
- **Dugite:** Iterates only `committee_hot_keys` — members without hot key authorization are invisible
- **Crate:** dugite-node (node/query.rs:277-313)
...
- **Dugite:** `encoding.rs:1055` always encodes `enc.u8(0)` regardless of actual type
- **Crate:** dugite-network (n2c/query/encoding.rs), dugite-ledger (state/mod.rs:204)
- **Source:** agent-memory/network-lead/committee-state-encoding-bugs.md
...
- **Dugite:** Uses u64 for stakes instead of Rational; definite arrays; missing WithOrigin; missing network_magic; doesn't filter big ledger peers (top 90% by stake); doesn't compute accumulated stake
- **Crate:** dugite-network (n2c/query/encoding.rs, encode_ledger_peer_snapshot)
...
- **Dugite:** Offers V16-V22 (tag 39 GetDRepDelegations not implemented)
- **Crate:** dugite-network
...
- **Dugite:** Plutus evaluation via `uplc` crate is present but cost model validation against cardano-node not confirmed; complex scripts may diverge
- **Crate:** dugite-ledger (plutus.rs)
...
- **Dugite:** Uses Taylor series for ln — different fixed-point truncation causes boundary disagreements with Haskell
- **Crate:** dugite-crypto (vrf.rs) — the vrf.rs code uses continued fraction for lncf, but activeSlotLog uses f64 path
...
- **Dugite:** Single encoding path; unclear if it correctly handles version gating
- **Crate:** dugite-network
...
- **Dugite:** Status unknown — likely still served to all clients
- **Crate:** dugite-network
...
- **Dugite:** Likely not gated by negotiated version
- **Crate:** dugite-network
...
- **Dugite:** Only `drep` is used; other prefixes missing from CLI
- **Issue:** #114
- **Crate:** dugite-cli (governance.rs)
...
- **Dugite:** May have differences in field names, formatting, or structure
- **Issue:** #118
- **Crate:** dugite-cli

### M-6: CLI does not infer network name from magic number
- **Spec:** cardano-cli infers "mainnet"/"preview"/"preprod" from magic; Dugite requires explicit `--network`
- **Issue:** #120
- **Crate:** dugite-cli
...
- **Dugite:** `calculate_change` handles multi-asset structurally but auto-balance coin selection is ADA-centric
- **Crate:** dugite-cli (transaction.rs)
...
- **Dugite:** Returns simplified state summaries, not full serialized CBOR
- **Crate:** dugite-network
...
- **Dugite:** LoE gates volatile-to-immutable flush (correct) but blocks are still applied to ledger regardless
- **Crate:** dugite-node (gsm.rs)
...
- **Dugite:** Simplified PeerManager with EWMA latency and reputation; no genesis-mode peer targets (40 BLP established, 30 active); no churn governor
- **Crate:** dugite-network
...
- **Dugite:** Wire format verified through integration tests only; no automated CDDL validation
- **Crate:** dugite-serialization (cddl_conformance.rs tests are structural, not spec-verified)
- **Source:** capability_gaps.md
...
- **Dugite:** Formula confirmed correct vs Koios epoch 1235 synthetic validation; zero tests against actual per-delegator on-chain amounts for real historical epochs
- **Crate:** dugite-ledger
- **Source:** capability_gaps.md
...
- **Dugite:** V16 in handshake but CSJ/GDD disabled; no bootstrap peer target switching
- **Issue:** #101
- **Crate:** dugite-network, dugite-node
...
- **Dugite:** MsgReplyTxs path exists but E2E flow unverified
- **Issue:** #102
- **Crate:** dugite-network
...
- **Dugite:** Encoding present but not golden-tested against Haskell output for all predicate failure types
- **Crate:** dugite-network
