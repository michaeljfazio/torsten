...
# Dugite State Assessment — 2026-03-18 (Soak Test Analysis)
...
- Note: metric names are `dugite_*` prefix (not `cardano_node_*`). Dashboards need updating.
...
### MEDIUM Priority
4. **Prometheus metric naming**: Uses `dugite_*` prefix instead of `cardano_node_*`. Not compatible with existing Grafana dashboards for cardano-node.
5. **dugite-lsm at mainnet scale**: Validated at ~3M UTxOs (preview). Not tested at 20M+ (mainnet). Need compaction + WAL crash recovery benchmarks.
...
2. dugite-lsm at 20M+ UTxO scale
