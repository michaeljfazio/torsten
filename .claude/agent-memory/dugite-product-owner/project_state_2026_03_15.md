...
# Dugite State Assessment — 2026-03-15 (Updated: Tranche 5 Planning)
...
- dugite-lsm custom LSM engine (replacing cardano-lsm)
...
- dugite-tui dashboard (ratatui, 12 tests)
...
**Mission:** Get Dugite to a state where a technically capable SPO can run it on mainnet
...
4. cncli compatibility: SPOs depend on it for leader log; Dugite has no cncli compatibility layer
...
- cncli queries the node via N2C LocalStateQuery; Dugite already implements all required tags
- Need: validate that cncli 6.x can connect to a running Dugite node and:
  - `cncli ping` returns success
  - `cncli sync` syncs blocks from Dugite's N2C socket
  - `cncli leaderlog` computes leader schedule using Dugite's VRF/epoch data
...
- Each test: capture real CBOR from Haskell cardano-node, assert Dugite encodes identically
...
- Run Dugite as active block producer on preview testnet for 2 full epochs
...
- Dugite only clears on rollback — correctness gap for block producers
...
- cncli compat unknown: SPOs can't use Dugite without it regardless of other correctness
