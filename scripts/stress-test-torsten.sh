#!/bin/bash
# Stress test: 5000 valid transactions via torsten-cli only
#
...
FEE=180000
WORK_DIR="/tmp/stress-test-torsten"

echo "=== Torsten 5K TX Stress Test (torsten-cli only) ==="
...
PRE_VALIDATED=$(curl -s http://localhost:12798/metrics | grep "^torsten_transactions_validated_total " | awk '{print $2}')
...
for tf in $(ls "$WORK_DIR/tx/"*.signed 2>/dev/null | sort); do
    ...
    if [ $((total % 1000)) -eq 0 ]; then
        ...
        mempool=$(curl -s http://localhost:12798/metrics 2>/dev/null | grep "^torsten_mempool_tx_count " | awk '{print $2}')
